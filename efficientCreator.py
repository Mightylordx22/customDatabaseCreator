...
def validInput(type):
    ...
    elif type == "varchar" or type == "char" or type == "integer" or type == "float":
        while True:
            try:
                rowName = input(f"What is the name of the {type.upper()}?\n>>> ").strip()
                if " " in rowName or rowName == "":
...
def createTable(tblName):
    ...
    if pKey in ["YES","Y"]:
        pKey = validInput("primary key")
        table.append(["PRIMARY KEY",[pKey]])
    else:
        print("Not adding a primary key!")
    carryOn = True
    while carryOn is True:
        additional = input("What else would you like in the table?\nA| VarChar / Char\nB| Integer / Float\nX| Nothing\n\n>>> ").strip().upper()
        if additional == "A":
            while True:
                whatVariable = input("What variable would you like?\n>>> ").strip().lower()
                if whatVariable not in ["varchar","char"]:
                    print("Sorry Varchar or Char only!\n")
                else:
                    break
            variable = validInput(whatVariable)
            chars = validInput("number")
            index = findIndex(table, f"{whatVariable}({chars})")
            if index is None:
                table.append([f"{whatVariable}({chars})", [f"{variable}"]])
            else:
                table = addID(table, f"{whatVariable}({chars})", f"{variable}")

        elif additional == "B":
            while True:
                whatVariable = input("What variable would you like?\n\n>>> ").strip().lower()
                if whatVariable not in ["integer","float"]:
                    print("Sorry integer or float here.")
                else:
                    break
            variable = validInput(whatVariable)
            index = findIndex(table, f"{whatVariable}")
            if index is None:
                table.append([f"{whatVariable}", [f"{variable}"]])
            else:
                table = addID(table, f"{whatVariable}", f"{variable}")


        elif additional == "X":
            print("Not adding anything else")
            print(table)
            carryOn = False
        else:
...
>>>>>>> b4512d403e2821b0b6d583e56eddad3a3518f67b
